...
### A10. 퀄리티 베이스라인 통과율 (v1.1 · E5)
`qa-validator` 실행 결과를 **통과율 %**로 표시. 각 항목 1점 집계.
- [ ] 파트별 bullet 수 3~5 범위
- [ ] bullet-text 문자 수 20~30
- [ ] SVG 중복률 ≤5%
- [ ] 실습 키트 체크포인트 ≥3
- [ ] script_parts 분량 오디언스 budget ±30% 이내
- [ ] 출처 ID `[src:N]` 부착률 ≥95%
- [ ] 골드 샘플 구조 유사도 ≥80%

릴리스 기준: **95% 이상 통과**. 위반 시 재실행 (최대 2회).

### A11. 접근성 (v1.1 · F3)
- [ ] 모든 `<svg>`에 `<title>` 존재
- [ ] 색상 대비 WCAG AA 통과 (본문 4.5:1·큰 텍스트 3:1)
- [ ] `<meta name="color-scheme" content="dark">` 포함
- [ ] `prefers-reduced-motion` CSS 분기 적용
- [ ] 모든 인터랙션 `aria-label` 또는 의미 있는 텍스트
- [ ] 키보드만으로 풀 플로우 완수 (수동 E2E 1회)
